...
#define SLDER_POS_FILTER_MAX_AGE                    1000    /**< Slider IIR filter max age in ms */
...
#define TOUCHPAD_BASELINE_RESET_COUNT_THRESHOLD     50000   /**< samples count number; All channels; */
...
#define TOUCHPAD_NOISE_THRESHOLD_PERCENT            0.40    /**< 40%; The threshold is used to determine whether to ignore the sample.
...
/*
 * Parameters for baseline determination.
 */
#define TOUCHPAD_BASELINE_UPDATE_SAMPLES	250	/* number of samples to update baseline and margins */
#define TOUCHPAD_BASELINE_NOISE_LEVEL		0.20	/* 20% cut off outliers past this for baseline updates */
...
typedef struct {
    ...
    uint16_t touch_count;	// number of times touched
    uint16_t noise_min;         // min sample counted as noise
    uint16_t noise_bl_min;      // min value to factor into baseline
    uint16_t noise_bl_max;      // max value to factor into baseline
    uint16_t noise_samples;	// number of samples in total_noise
    uint32_t total_noise;       // total of noise samples in current period
...
static uint32_t tp_noise_sample_count;

/*
 * Determine if touchpad sample is in the noise.
 */
static inline int tp_pad_is_noise(tp_dev_t *tp_dev, uint16_t val)
{
	return val >= tp_dev->noise_min;
}

/*
 * Accumulate noise data for touchpad for use in setting the next baseline.
 * Limit the value if it is outside of the noise range.
 */
static void tp_noise_accum(tp_dev_t *tp_dev, uint16_t val)
{
	if (val < tp_dev->noise_bl_min) {
		val = tp_dev->noise_bl_min;
	} else if (val >= tp_dev->noise_bl_max) {
		val = tp_dev->noise_bl_max;
	}
	tp_dev->total_noise += val;
	tp_dev->noise_samples++;
}

/*
 * Init noise thresholds from new baseline.
 */
static void tp_noise_init(tp_dev_t *tp_dev)
{
	uint16_t base = tp_dev->baseline;
	unsigned int noise;

	noise = base * tp_dev->touch_thr * TOUCHPAD_NOISE_THRESHOLD_PERCENT;
	tp_dev->noise_min = base - noise;

	noise = base * tp_dev->touch_thr * TOUCHPAD_BASELINE_NOISE_LEVEL;
	tp_dev->noise_bl_min = base - noise;
	tp_dev->noise_bl_max = base + noise;
	tp_dev->total_noise = 0;
	tp_dev->noise_samples = 0;
}

/*
 * Recompute baseline for touch pad.
 */
static void tp_noise_baseline_set(tp_dev_t *tp_dev)
{
	if (tp_dev->noise_samples) {
		tp_dev->baseline = tp_dev->total_noise / tp_dev->noise_samples;
	}
	tp_noise_init(tp_dev);
}
...
static uint32_t _slider_filter_iir(uint32_t in_now, uint32_t out_last, uint32_t k)
{
    ...
    if (k == 0) {
        ...
        now = esp_timer_get_time();	/* time in microseconds */
        age = now - last_time;
        last_time = now;
        if (age > SLDER_POS_FILTER_MAX_AGE * 1000) {
            return in_now;
        }
...
void filter_read_cb(uint16_t raw_data[], uint16_t filtered_data[])
{
    ...
    for (int i = 0; i < TOUCH_PAD_MAX; i++) {
	    ...
	    tp_noise_accum(tp_dev, raw_data[i]);
	    sample_taken = 1;
            ...
            if (TOUCHPAD_STATE_IDLE == tp_dev->state || TOUCHPAD_STATE_RELEASE == tp_dev->state) {
                ...
                if (tp_pad_is_noise(tp_dev, raw_data[i])) {
                ...
                if (!tp_pad_is_noise(tp_dev, raw_data[i]) && tp_dev->diff_rate > tp_dev->touch_thr - tp_dev->hysteresis_thr) {
                    tp_dev->debounce_count = 0;
            ...
            if (!tp_pad_is_noise(tp_dev, raw_data[i]) && tp_dev->diff_rate > tp_dev->slide_trigger_thr && tp_dev->button_type >= TOUCHPAD_LINEAR_SLIDER) {
    ...
    if (sample_taken) {
        tp_noise_sample_count++;
        if (tp_noise_sample_count >= TOUCHPAD_BASELINE_UPDATE_SAMPLES) {
            tp_noise_sample_count = 0;
            for (int i = 0; i < TOUCH_PAD_MAX; i++) {
                tp_dev_t *tp_dev = tp_group[i];
                if (tp_dev) {
                    tp_noise_baseline_set(tp_dev);
                }
            }
        }
    }
...
int iot_tp_slide_counts_get(tp_slide_handle_t tp_slide_handle, int *counts, unsigned int ncounts)
{
    tp_slide_t *tp_slide = (tp_slide_t *) tp_slide_handle;
    tp_dev_t *tp_dev;
    unsigned int i;
    ...
    if (ncounts >= tp_slide->tp_num) {
        ncounts = tp_slide->tp_num;
    }
    for (i = 0; i < ncounts; i++) {
        tp_dev = tp_slide->tp_handles[i];
        counts[i] = tp_dev ? tp_dev->touch_count : -1;
    }
    return ncounts;
}
